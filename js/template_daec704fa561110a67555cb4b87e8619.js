...
var index_page = {
	...
	init: function() {
		...
		index_page.i_timer = setInterval(index_page.image_change, 5000);
		if($('.index_image').size() < 2){
			clearInterval(index_page.i_timer);
			$('.index_image').css({'opacity': 1});
		}
	},
	...
	image_change: function() {
		//var prev = $('.index_image.cur');
		//var ind_prev = $('.index_image').index(prev);
		//var ind_next = (ind_prev < $('.index_image').size() - 1) ? (ind_prev * 1 + 1) : 0;
		//var next = $('.index_image').eq(ind_next);
		//prev.animate({'opacity': 0}, 1000);
		//next.animate({'opacity': 1}, 1000, function() {
		//	prev.css({'zIndex': 0}).removeClass('cur');
		//	next.css({'zIndex': 1}).addClass('cur');
		//});
		//$('.index_images_in .index_image').eq(0).fadeOut(1000,function(){
		//	$('.index_images_in').prepend($('.index_images_in .index_image').eq(0));
		//})
		var len = $('.index_images_in .index_image').length;
		$('.index_images_in .index_image').eq(len-1).animate({"opacity":"0"},1000,function(){
			$('.index_images_in').prepend($('.index_images_in .index_image').eq(len-1));
			$('.index_images_in .index_image').eq(0).css({"opacity":"1"});
		})
	},
